...
export const WhyAttend = ({ title, text, image, component }: WhyAttendProps) => (
  <section className="bg-white border-b py-8">
    <div
      ...
      style={{
        flexDirection: component.variant === 'whyattendleft' ? 'row' : 'row-reverse',
      }}
    >
      {image ? (
        <div className="w-1/2">
          <img src={image} alt={title} width={400} height={400} loading="lazy" className="p-10" />
        </div>
      ) : null}
      <div className="w-1/2">
        <div className="p-10">
          ...
          />
          <hr />
          <p className="text-gray-800 p-10 whitespace-pre-line" dangerouslySetInnerHTML={{ __html: text }} />
        </div>
      </div>
    </div>
